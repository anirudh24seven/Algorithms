...
int *constructSegmentTreewrapper (int inputArray[], int inputArraySize);
...
int main() {
	...
	int *segmentTree = constructSegmentTreeWrapper(inputArray, n);
...
int* constructSegmentTreeWrapper (int inputArray[], int inputArraySize) {
	...
	int maxSize = (int)pow(2,height) - 1;
...
int constructSegmentTreeCore (int inputArray[], int startPosition, int endPosition, int* segmentTree, int currentNode) {
	...
	segmentTree[currentNode] = constructSegmentTreeCore(inputArray, startPosition, middlePosition, segmentTree, currentNode*2+1) + constructSegmentTreeCore(inputArray, middlePosition+1, endPosition, segmentTree, currentNode*2+2);
...
void updateSegmentTreeCore(int *segmentTree, int startPosition, int endPosition, int k, int difference, int index) {
	...
	if(startPosition != endPosition) {
		int middlePosition = (startPosition + endPosition)/2;
		updateSegmentTreeCore(segmentTree, startPosition, middlePosition, k, difference, 2*index+1);
		updateSegmentTreeCore(segmentTree, middlePosition+1, endPosition, k, difference, 2*index+2);
	}
}